...
class Affine2DCoordinateTransform(CoordinateTransform):
    ...
    def reverse(self, coord_labels):
        ...
        results = {}
        for dim, positions in zip(self.xy_dims, [x_positions, y_positions], strict=False):
...
class GeoTransformIndex(CoordinateTransformIndex):
    ...
    def from_grid_mapping(
        ...
        *,
        ...
        geotransform = np.fromstring(grid_mapping.attrs["GeoTransform"], sep=" ").tolist()
        ...
        return cls(transform=xtransform, grid_mapping=grid_mapping, coord_names=coord_names)
    ...
    def isel(self, indexers) -> Self:
        ...
        new_spatial_ref.attrs["GeoTransform"] = " ".join(map(str, new_transform.to_gdal()))
        new_index = type(self)(
    ...
    def sel(self, labels: dict[Any, Any], method="nearest", tolerance=None, crs=None):
        ...
        transformer = transformer_from_crs(crs_from=qcrs, crs_to=self.crs, always_xy=True)
