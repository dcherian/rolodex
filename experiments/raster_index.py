...
def compute_bounding_box_affine(A1: Affine, A2: Affine, *, how: str):
    _assert_transforms_are_compatible(A1, A2)

    # Define the unit square corners (in 2D)
    corners = [(0, 0), (1, 1)]

    # Apply both affine transformations to the corners
    transformed1 = np.stack([A1 * point for point in corners])
    transformed2 = np.stack([A2 * point for point in corners])
    ...
    if how == "outer":
        ...
        Nx = None if A1.a == 0 else int(np.abs((max_x - min_x) / A1.a) - 1)
        Ny = None if A1.e == 0 else int(np.abs((max_y - min_y) / A1.e) - 1)
    else:
        raise NotImplementedError

    Anew = Affine(
        A1.a, A1.b, min_x if A1.a > 0 else max_x, A1.d, A1.e, min_y if A1.e > 0 else max_y
    )
    return Anew, Nx, Ny
...
class AxisAffineTransform(CoordinateTransform):
    ...
    def __repr__(self) -> str:
        return f"<AxisAffineTransform(size={self.size!r}, coord_name={self.coord_name!r}, dim={self.dim!r}, is_xaxis={self.is_xaxis!r}, affine={self.affine!r})>"
...
class AxisAffineTransformIndex(CoordinateTransformIndex):
    ...
    # TODO: duplicates self.transform
...
class RasterIndex(Index):
    ...
    def from_transform(
        ...
        if affine.is_rectilinear and affine.b == affine.d == 0:
            # TODO: confusing that the same affine is passed. Could set Δx, Δy=0 as appropriate
    ...
    def join(self, other, how) -> "RasterIndex":
        ...
        ours = self.combined_affine_transform()
        theirs = other.combined_affine_transform()

        new_affine, Nx, Ny = compute_bounding_box_affine(ours, theirs, how=how)

        import ipdb

        ipdb.set_trace()
        # new_indexes = {
        #     axis: this.join(other._wrapped_indexes[axis], how=how)
        #     for axis, this in self._wrapped_indexes.items()
        # }
